...
from ..pcxgan import modules_mask as modules
import numpy as np
import time
...
def main(flags):
  ...
  def build_and_train_model(rank, num_nodes, train_fold, test_fold, flags):
      ...
      train_data = data_loader.DataGenerator_DistribMask(flags, flags.data_path, if_train=True).load()
      test_data = data_loader.DataGenerator_DistribMask(flags, flags.data_path, if_train=False).load()
