...
from . import modules_old as modules
import evaluate
import loss
import pandas as pd
...
class GAN_UVIT(keras.Model):
	def __init__(self, flags):
		...
		self.disc_loss_coeff = flags.disc_loss_coeff #10
	...
	def train_generator(self, time_input, ct, mri, discriminator_loss):
		...
		with tf.GradientTape() as tape:
			...
			"""
			It's passed from the previous step in the training

			calcu;ate the loss of the disc_
			for real and for fake
			get the avg(real, fake)
			real_d_output = self.discriminator([ct, mri])
			"""
			
			_, fake_image = self.network(
				[ct, time_input, mri]
			)
	...
	def test_step(self, data):
		...
		#real_d_output = self.discriminator([ct, mri])
		_, fake_image  = self.network([ct, t, mri])
